...
class DefaultConfig(object):
    def __call__(self, parser: ArgumentParser) -> ArgumentParser:
        ...
        parser.add_argument('-nd', '--dim-number', type=int, default=1024)
        parser.add_argument('-nad', '--a-dim-number', type=int)
        parser.add_argument('-nns', '--n-samples-num', type=int, default=1000)
        parser.add_argument('-nas', '--a-samples-num', type=int, default=10)
        parser.add_argument('-no', '--n-other', type=int, default=10,
        ...
        parser.add_argument(
            '--loss-weights', type=float, nargs='*', default=[1., .0, 0.0, 1.0],
...
class DefaultGenerated(DefaultConfig):
    def __call__(self, parser: ArgumentParser) -> ArgumentParser:
        ...
        parser.set_defaults(
            a_mean=1, a_std=0.06, a_dim_number=1, anom_dims=2
        )
        return parser
