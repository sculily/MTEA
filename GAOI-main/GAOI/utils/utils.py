...
def data_plot(data, labels, new_point=None, dimensions=None, name='gen_points', save=True, show=False, train=False,
    ...
    for i in range(len(dimensions)):
        ...
        plt.scatter(data[labels == 0, d], np.full_like(data[labels == 0, d], fill_value=1.5), c='cornflowerblue', edgecolors='royalblue',
                    label='normal')
        plt.scatter(data[labels == 1, d], np.ones_like(data[labels == 1, d]), c='#C0Ca33', edgecolors='#AFB42B',
                    label='outlier')
        if train:
            plt.scatter(new_point[d], np.full_like(new_point[d], fill_value=0.5), c='coral', edgecolors='orangered',
                        label='new point')
            yticksp_l = [0.5, 1., 1.5]
            yticksl_l = ['new point', 'outlier', 'normal']
        else:
            yticksp_l = [1., 1.5]
            yticksl_l = ['outlier', 'normal']
...
def getclass(ti, tl, current_class, other_class):
    ...
    n = ti.shape[0] # 101
    #print("n",n)
    it = np.full(n, False) # [101个false]
    #print("it",it)
    for c in classes:
        jt = tl == c # 1 jt = [false....,true] 0 [true....,flase]
        it = it | jt # it = [false....,true] 0 it = [true....,true]
    tl = tl[it]
...
def getneighds(img_id, train_images, train_labels, classes, ns=0, no=10, columns=None):
    """
    ...
    print("***Img Label:", classes[cl]) # 1
    cloth = (cl + 1) % 2
    print("***Img Other Label:", classes[cloth]) # 0
...
def cross_loss(y_true, y_pred):
    ...
    return -K.sum(pt_0 * K.log(pt_1)) - K.sum(
         (1. - pt_1) * K.log(1. - pt_0))
